...
namespace QueryBuilder2
{
    public class QueryBuilder : IDisposable
    {
        ...
        /// </summary>
        /// <param name="obj">Object of type T to be updated (its Ide param will be used to find it)</param>
        public void Update<T>(T obj) where T : IClassModel
        {
            ...
            var command = connection.CreateCommand();
            command.CommandText = $"UPDATE {typeof(T).Name} SET {sb1} WHERE Id = {obj.Id}";
            var reader = command.ExecuteNonQuery();
        }
